...
"""def apply_threshold_segmentation(image):
    threshold_value = threshold_otsu(image)
    binary_image = image > threshold_value
    return binary_image.astype(np.uint8)  # Convert to uint8 (0 or 1)"""
# Function to apply threshold segmentation to grayscale image
"""def apply_threshold_segmentation(image):
    threshold_value = threshold_otsu(image)
    binary_image = image > threshold_value
    return binary_image.astype(np.uint8)  # Convert to uint8 (0 or 1)"""
def apply_threshold_segmentation(image):
    ...
    if threshold_value is None:
        raise ValueError("Threshold value is None. Check the input image.")
    binary_image = image > threshold_value
    return binary_image.astype(np.uint8)  # Convert to uint8 (0 or 1)
def apply_threshold_segmentation(image):
    threshold_value = 127
    if threshold_value is None:
        # Handle the case where threshold value is None (e.g., image is too uniform)
...
try:
    ...
    for i, image_name in enumerate(no_tumor_images):
        if image_name.split('.')[1] == 'jpg':
            ...
            #image = anisodiff(image,niter=150, kappa=10)
            #Apply anisotropic filter to each channel separately
            red_channel = anisodiff(image[:, :, 0], niter=150, kappa=10)
            green_channel = anisodiff(image[:, :, 1], niter=150, kappa=10)
            blue_channel = anisodiff(image[:, :, 2], niter=150, kappa=10)           # Combine the channels back to a 3D image
           # image = np.stack((red_channel, green_channel, blue_channel), axis=-1)
            # Apply threshold segmentation
            image = apply_threshold_segmentation(image)
    ...
    for i, image_name in enumerate(yes_tumor_images):
        if image_name.split('.')[1] == 'jpg':
           ...
           # image = Image.fromarray(image, 'RGB')
            ...
            red_channel = anisodiff(image[:, :, 0], niter=150, kappa=10)
            green_channel = anisodiff(image[:, :, 1], niter=150, kappa=10)
            blue_channel = anisodiff(image[:, :, 2], niter=150, kappa=10)"""
            # Combine the channels back to a 3D image
           # image = np.stack((red_channel, green_channel, blue_channel), axis=-1)
            # Apply threshold segmentation
            image = apply_threshold_segmentation(image)
            # Apply morphological operation (e.g., erosion or dilation)
